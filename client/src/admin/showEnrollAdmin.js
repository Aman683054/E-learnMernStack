...
export default class EnrollList extends Component {
    ...
    delete(id){
        console.log(id);
        axios.delete('http://localhost:5000/enrollment?id='+id)
          .then((result) => {
            // this.forceUpdate()
    ...
    render() {
        ...
        let filteredusers=this.state.todos.filter(
            (enroll)=>{
                return (enroll.student.email.indexOf(this.state.search)!==-1||
        ...
        <div>
               ...
               <a href="/createEnrollAdmin" className="btn btn-primary btn-info btn active" role="button" aria-pressed="true">Create Enrollment</a> <br/>
               
                <h1 style={{marginLeft:"-200px", textDecoration:"underline", color:"#F0542D"}}>Enrollment List</h1>
